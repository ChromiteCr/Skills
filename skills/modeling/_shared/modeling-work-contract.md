...
| `R` | 运行 | 字段见 `code-reproducibility-checklist.md` §7 Run Manifest（唯一 schema） |
...
5. 运行状态写在 Run Manifest 的 `status` 字段（`not-run | failed | passed-with-warnings | passed`，见
	`code-reproducibility-checklist.md` §7）；不与产物或证据状态混用。
...
- 能写文件：修改前保留原内容与改动范围；不能写：在对话中返回候选产物。课程作业、竞赛等受评任务在规则明确允许
  完整 AI 生成之前，以及没说明是否受评时，按各 skill 的代写边界裁剪：代码只给实现合同、伪代码、测试与对学生已有
  代码的局部补丁，不给可直接提交的完整产物。
